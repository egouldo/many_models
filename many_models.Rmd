...
- `broom::tidy` model component-level statistics: coefficient estimates
- `broom::augment` observation-level -fitted values, residuals. .
- `broom::glance` - model-level statistics: e.g. ${R}^{2}$ , F-stat, deviance.

```{r demo-broom-functions}
library(tidyverse)
data("diamonds")
lm_diamonds <- lm(price ~ carat, data = diamonds)
lm_diamonds %>% broom::tidy() # one observation per model term
lm_diamonds %>% broom::augment() # one observation per observation in the original data
lm_diamonds %>% broom::glance() # one observation per model
```
